...
impl ReaderHalf {
    ...
    pub async fn read_u32(&mut self) -> tokio::io::Result<u32> {
        match self {
            Self::ServerTls(t) => t.read_u32().await,
            Self::ClientTls(t) => t.read_u32().await,
            Self::Regular(t) => t.read_u32().await,
        }
    }
...
impl WriterHalf {
    pub async fn write_u32(&mut self, n: u32) -> tokio::io::Result<()> {
        ...
        match self {
            Self::ServerTls(t) => t.write_u32(n).await,
            Self::ClientTls(t) => t.write_u32(n).await,
            Self::Regular(t) => t.write_u32(n).await,
        }
    }
